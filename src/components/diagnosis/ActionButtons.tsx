...
import { ShoppingCart, DownloadCloud, SendHorizontal, ShieldCheck, Video, ArrowLeft } from "lucide-react";
...
export const ActionButtons = () => {
  ...
  const handleBackToDiagnosis = () => {
    // 診断画面のstep4（AI対話）に戻る
    // state引数でgoToStep4フラグをtrueに設定することで、STEP4に直接ジャンプするように指示
    // チャット履歴を保持するためのフラグも追加
    navigate('/diagnosis', { 
      state: { 
        goToStep4: true,
        preserveChat: true 
    ...
    <div className="space-y-4">
      <div className="bg-white shadow-sm rounded-lg border-4 border-[#40414F] p-4">
        ...
        <div className="grid gap-3">
          ...
          <Button 
            className="flex justify-start items-center gap-3 bg-blue-600 hover:bg-blue-700 text-left"
            onClick={handleBackToDiagnosis}
          >
            <ArrowLeft />
            <div>
              <div className="font-medium">AIと対話に戻る</div>
              <div className="text-sm font-normal opacity-80">STEP4のAIとの対話画面に戻ります</div>
            </div>
          </Button>
        </div>
      </div>
